...
import { AiOutlineFieldTime, AiOutlineEye, AiOutlineHeart } from "react-icons/ai";
...
const EditRecipe = () => {
  const { id } = useParams();
  const { data,error:recipeError } = useGetRecipeQuery(id);
...
const { data :updateData, error, isLoading } = result;


useEffect(()=>{
  if (recipeError) {
    toast.error(recipeError.data.msg)
    navigate("/404");
    return;
  }
},[recipeError,navigate])
const categories = [
  "American",
  "Indian",
  "French",
...
const {
  ...
  initialValues: {
    ...
    avgCookingTime: data?.recipe?.avgCookingTime,
  },
...
useEffect(() => {
  ...
  setPreImg(data?.recipe?.image);
  setFieldValue("title", data?.recipe?.title);
  setFieldValue("desc", data?.recipe?.desc);
  setFieldValue("ingredients", data?.recipe?.ingredients);
  setFieldValue("category", data?.recipe?.category);
  setFieldValue("avgCookingTime", data?.recipe?.avgCookingTime);
...
useEffect(() => {
  ...
  image  && image !== data?.recipe?.image && fileReader();
}, [image]);
...
return (
  <section className="flex  items-center justify-center bg-rose-100 py-10 px-2 ">
    <RHelmet title="Update Recipe : Mine Recipes" />
      ...
      >
        ...
        <div className="flex w-full flex-row-reverse justify-between items-center">
          ...
          <button
            ...
            disabled={isLoading}
          >
            {isLoading ? <Spinner /> : "Update Recipe"}
          </button>
        </div>
      </form>
    </div>
